...
“注册能力”是运行时内部的执行与安全边界，不是交给同伴模型枚举匹配的命令表。模型表达具身意图和期望结果，不直接选择技能或按对象类别调用动作。语言中的“我”“那里”“这棵”等指代先经过有证据的 Grounding；Behavior Synthesizer 再使用通用、与对象类别无关的操作器形成计划。允许硬编码的是身体原语、物理约束和安全不变量，不是玩家措辞、对象种类或任务名称。
...
9. 仅在需要时加载的游戏知识和经验证行为经验；不包含可调用操作器目录。
...
- 原始世界数据先压缩为认知观察。
- 旧聊天通过情景记忆检索，不无限累积。
- 详细游戏知识与经验证行为经验按需加载；内部操作器名不进入模型上下文。
...
→ Action Runtime：校验操作器、资源锁、取消与安全约束
→ Motor Controller：执行通用身体原语
...
模型不直接调用 Action Runtime。Behavior Synthesizer 生成的内部计划只能使用注册操作器；每个操作器必须声明：

```ts
interface BehaviorOperatorDefinition<Args, Result> {
  name: string
  inputSchema: unknown
...
操作器按身体效果命名并消费 grounded handle 或通用空间约束，例如改变朝向、改变移动输入或执行准星交互；不得按 `player`、`block`、`tree` 等对象类别命名。它们不进入主同伴模型的可调用目录。结果统一包含状态、持续时间、实际效果、观察、副作用和失败原因。
...
- 身体原语/操作器：一个通用、与对象类别无关、可验证的执行能力。
- 行为计划：Behavior Synthesizer 为当前 grounded intent 临时形成的有界组合，可取消、有超时和失败路径。
- 长期活动：由 Companion Runtime 维护，跨多个决策和短流程推进。

行为计划可以使用受约束 AST，但不作为固定命令菜单暴露给主模型，也不因成功一次就自动注册为对象专用技能。长期活动绝不能编译为一个运行数小时、无法交谈的巨大 Flow。
...
系统按 Minecraft 版本、环境、具身目标特征和内部行为模式统计成功率、常见失败和平均耗时。可靠性数据影响身体可供性摘要与行为合成，但不能把失败静默隐藏，也不能因此自动生成对象专用 model-facing skill。
...
Action lane       内部行为计划与身体原语，按身体资源协调
...
├── behavior/        # synthesize internal operator plans from grounded embodied intent
...
├── skills/          # v0.1 compatibility adapters; no new model-facing object skills
...
`src/perception/`、`src/grounding/`、`src/behavior/`、`src/motor/`、`src/navigation/`、`src/actions/` 和 `src/skills/` 均不得导入 Mineflayer/Prismarine 原始类型；唯一适配入口及迁移规则见 ADR 0005。
...
5. 具身意图必须经过 Grounding 与 Behavior Synthesis；执行只使用内部注册操作器和有界计划，不运行任意模型代码。
...
5. Grounding、Behavior Synthesizer、Action Runtime、身体资源锁与首批通用操作器契约。
