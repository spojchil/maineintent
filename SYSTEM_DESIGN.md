...
完整的版本化 schema、具身意图 Grounding、内部计划预检、过期拒绝、输出修复和效果提交语义见 [同伴决策协议与上下文包](./docs/design/decision-contract-and-context.md)。本节保留系统级概览。
...
v0.2 先固定[合法信息接口、Help 发现与 UI 会话](./docs/design/information-access-and-ui.md)，并以[统一 Information Runtime](./docs/design/information-runtime.md)作为唯一实现入口。具身控制的调研清单保留在[具身能力与接口清单](./docs/design/embodied-interface-inventory.md)，但其中曾提出的逐 tick `BodyInputPlan` 不再是接受的系统契约。
...
`src/information/`、`src/perception/`、`src/grounding/`、`src/behavior/`、`src/motor/`、`src/navigation/` 和 `src/actions/` 均不得导入 Mineflayer/Prismarine 原始类型；唯一适配入口见 ADR 0005。Information Runtime 的模块契约见 [统一 Information Runtime](./docs/design/information-runtime.md)。项目不保留原型 `src/skills/` 兼容目录。
