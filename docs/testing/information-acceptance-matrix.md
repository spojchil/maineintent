...
2. **诚实**：不能确定或当前不可检查时返回明确 unavailable/unknown，不用内部真值补齐；
3. **有界**：字段数、分页、字节、时间、引用和缓存均受限；
4. **可失效**：连接、世界、维度、screen、revision 或权限改变后，旧值和引用确定性失效；
...
- 显示精度和量化边界；
- unsupported、unknown、未加载与不可见的区别；
...
- Provider 的额外字段、错误 source、异常、超大结果和隐藏对象均转成 sanitized error。
...
| 接口族 | 正常场景 | unavailable/unknown 场景 | 主要禁漏项 |
|---|---|---|---|
...
| hotbar/inventory/tooltip | 选中槽、数量、可见 tooltip | screen 未开、item ref 陈旧、物品变化 | 私有 NBT、隐藏组件、其他容器 |
...
| 转换 | 必须变化 | 必须失效 |
|---|---|---|
| disconnected → connecting → configuration → play | connection state；新有效会话提升 epoch | 旧连接的所有 ref/cursor/value |
| world/dimension change | world/dimension scope 与相关 Provider revision | 旧世界/维度观察、分页和目标引用 |
| death → respawn | lifecycle/status revision；是否提升 epoch 由 lifecycle 契约固定 | 依赖旧身体/观察状态的值 |
...
| reduced-debug/权限变化 | Help availability/effective Catalog 或字段 revision | 已不再获准的读取与 continuation |
...
测试在 O0 中放入显眼且唯一的 canary，例如隐藏 saturation、小数坐标、私有 NBT 键、未打开容器物品名和 operator-only 标识。对 Catalog、Help、Read、错误、trace、Journal、Context、模型请求和 debug JSON 做递归扫描；任何命中都失败。
...
4. `information-viewport-optics`：前后方、墙、玻璃、树叶、水和未加载边界；
5. `information-sound`：方向、距离带、墙后、无位置声音和聚合；
6. `information-non-leakage`：canary 与 audience/旁路综合检查。
...
5. 正常、partial、unknown、unsupported、越权和超限是否都有用例？
