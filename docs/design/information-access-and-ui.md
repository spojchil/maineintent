...
- selector 只能回传给签发它的接口或 Help 明确声明的兼容接口；Runtime 校验 audience、epoch、world、screen instance 和有效期。
- cursor 同样 opaque，并绑定接口、字段集合、selector、revision 与调用者；不能修改、跨接口复用或在 revision 改变后继续翻页。
- 每次 Read 的字段数、`limit`、返回字节数和耗时有硬上限；超限返回 `invalid_page` 或截断后的 `nextCursor`，不能一次导出完整聊天史、全部配方或无限观察缓存。
...
| `current_screen_information` | 当前 screen 的可见槽位、控件、文本和进度 | 仅当前 screen instance |
...
自身背包可以结构化检查；未打开的外部容器、其他玩家背包和未打开末影箱不可读取。
...
- `connectionEpoch`、world、dimension 或 screen instance 改变会使相关 selector 失效。
