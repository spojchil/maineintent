...
interface ContextSource {
  kind: 'runtime' | 'event' | 'profile' | 'memory' | 'player' | 'skill_registry' | 'capability_registry' | 'summary'
  ids: string[]
  trust: ContextTrust
  observedAt?: string
  validAt?: string
...
- 只列本轮相关身体可供性；模型不能凭旧记忆要求当前不存在的能力，也不能输出内部操作器名。
- 所有截断和遗漏必须出现在元数据中。
...
- v0.2 的 `after_intent_*` 依赖具身意图 effect；内部操作器从不成为语言依赖对象。
...
- 主模型只看到身体可供性、限制和证据要求，不看到可调用 skill/operator 集合；任何内部操作器名出现在模型输出中都拒绝。
...
- 必需内容超预算失败，不静默丢弃。
- 恶意记忆或世界文本不能改变指令优先级、伪造 grounded referent 或调用内部操作器。
...
2. v1 `action_group` 只作为兼容输入存在，不允许注册新的对象专用 skill，并有明确移除版本。
