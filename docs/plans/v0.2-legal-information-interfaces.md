...
> 日期：2026-07-14
>
> 上游：[产品设计](../../PRODUCT_DESIGN.md)、[系统设计](../../SYSTEM_DESIGN.md)、[合法信息接口设计](../design/information-access-and-ui.md)、[Information Runtime 模块设计](../design/information-runtime.md)
>
> 里程碑：[v0.2 — 合法信息与界面接口](https://github.com/spojchil/mineintent/milestone/3)
> 追踪：[#58](https://github.com/spojchil/mineintent/issues/58)
...
- 统一 `InformationRuntime`、`information_catalog` 与 `information(interfaceId, operation)` 两个模型工具；
- 强类型 Provider、最小只读 source port、Registry、Access Policy 和 Tool Session 预算；
...
1. 以 `InformationRuntime` 作为唯一认知入口，建立 Registry、Access Policy、Ref/Cursor Store 与有界 Tool Session。
...
5. 让模型首次使用或 schema 变化时先 Help，相同 revision 可缓存；读取和分页具有字段数、字节数与时间上限。
...
4. 证明生产链路不通过 Backend snapshot、Safety 或 Paper 裁判旁路获取信息。
...
| 2 | [#53 定义合法信息目录、Help/Read 与字段版本](https://github.com/spojchil/mineintent/issues/53) | 无 |
