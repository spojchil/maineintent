...
>
> 里程碑：[v0.3 — 可信具身闭环](https://github.com/spojchil/mineintent/milestone/2)
>
...
当前实现进度（2026-07-22）：P0 所需的事件、取消、调试和 Paper 验收骨架已建立；P1 的有源相对坐标、opaque ref、Grounding、有限扫描、渐进注视与新观察结果验证已经通过自动测试和单轮 Paper 场景。P2 及以后尚未接入生产 Behavior 链路；P1 也仍需真人观察验收，不因自动场景通过而视为整个里程碑完成。
