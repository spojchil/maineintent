...
def system_prompt(profile_content: str) -> str:
    return (
        ...
        "viewport.visibleBlocks 是正前方视野内经过遮挡判定后确实可见的方块列表，坐标是相对自身的偏移量"
        "（offsetX/Y/Z，例如 offsetZ=-3 大致是前方3格），不是世界坐标，回答时按偏移量描述相对位置，"
        "不要把它当成精确坐标；列表为空是正常的（比如正对着空地），truncated 为 true 时说明视野内还有更多方块没列出。"
